...
const isCustomTriggerConfig = (
  config: Config
): config is CustomTriggerConfig => {
...
const getTextSubstring = (text: string, start: number, end?: number) =>
...
const getKeyword = ({
  ...
  let localCursor =
    part.position.start + getTextLength(firstPartTextSlice + config.trigger);
  ...
  while (localCursor <= cursor) {
    ...
    if (nextPartTextSlice != null) {
      ...
      if (localCursor >= cursor) {
        ...
        const charactersAfterTriggerLength =
          nextPlainTextPartLength - difference;
...
const generateValueWithAddedSuggestion = (
  ...
  return getValueFromParts([
    ...
    generatePlainTextPart(
      getTextSubstring(currentPart.text, 0, newMentionPartPosition.start)
    ),
...
const getMentionDataFromRegExMatch = (
  matchPlainText: string
): TriggerData | null => {
  const regexExecResult = triggerRegEx.exec(
    matchPlainText
  ) as any | null;
 ...
 */
const replaceTriggerValues = (
  value: string,
  replacer: (mention: TriggerData) => string
) =>
  value.replace(
    RegExp(triggerRegEx, 'g'),
    (fullMatch, original, trigger, name, id) =>
      replacer({
        original,
        trigger,
        name,
